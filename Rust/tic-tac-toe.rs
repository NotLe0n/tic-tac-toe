fn main() {
    ...
    while !check_win(board) {
        ...
        println!(
            "Please take a turn player {}",
            (if player_1_turn { "1" } else { "2" })
        );
        ...
        while read_int(&mut input).is_err() {
            println!("Incorrect input. Please only type characters from 1 to 9");
        }
...
fn read_int(num: &mut usize) -> Result<(), ()> {
    let mut input_line = String::with_capacity(3);

    std::io::stdin().read_line(&mut input_line).map_err(|_| {})?;
